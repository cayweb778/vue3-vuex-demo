  ...
  return {
    ...
    methods: {
      ...
      thisHelp(title) {
        ...
        $('#helpContent').html(context);
        layer.open({
          title: title
          , content: $('#choose-box-wrapper4').html()
          , btn: ['关闭']
          , area: ['800px', '410px']
          , tipsMore: true
          , btnAlign: 'c'
          , success: function() {
            $('.layui-layer-btn .layui-layer-btn0').addClass('dy_button');
            $('.layui-layer-content .widget').css('margin', '0');
          }
        });
      },
      openPrint() {
      ...
      switchVouch(num) {
        ...
        $.ajax({
          ...
          async: false,
          success: function(res) {
            let parms = res.obj.parms;
            let trs = [];
            let assist = [];
            for (let i in parms['vouchList']) {
              let tr = {};
    ...
    mounted() {
      ...
      if (window.screen.width < 1400) {
        $('#pageEditZoom').css('zoom', $('body').width() / $('#pageEditZoom').width() * 0.6);
      }

      let vueObj = this;
      setTimeout(function() {
        if (vueObj.title != 'look') {
          $('.voucher-item table td:first textarea').focus();
        }
      }, 100);
      $('.voucher-item').scroll(function() {
        $('.ulX').hide();
      });
      let _this = this;
