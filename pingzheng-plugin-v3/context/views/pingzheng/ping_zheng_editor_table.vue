...
const showPingZheng = computed(() => showPingZhengStore.gefocusTextEditorPingZheng);
...
function focusTextEditor(source,textEL) {
  if (this.focusTextEditorBan == true) return;
  setTimeout(function() {
    ...
    textEL.select()
    source.target.parentElement.style.display='none'
  });
}
...
function proprowsWatch(newVal, oldName) {
  ...
  for (const i in this.pingZhengModel.rows) {
    ...
    if ($.trim(this.pingZhengModel.rows[i].zhaiYao) != '' || $.trim(this.pingZhengModel.rows[i].kuaiJiKeMuFullName) != '') {
      this.getBalance(jquery('#voucherBill tbody tr:eq(' + i + ') td:eq(1) ul'), this.rows[i].kuaiJiKeMuFullName.split(' ')[0], i);
    }
